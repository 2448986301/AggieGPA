...
struct CourseEditorView: View {
    ...
    init(term: AcademicTerm, course: CourseRecord? = nil) {
        ...
        // A new course has no final report yet. NG is the explicit pending
        // state; current/projected values come from the gradebook and never
        // get written here.
        _grade = State(initialValue: course?.grade ?? .noGrade)
        ...
        NavigationStack {
            Form {
                Section("Required") {
                    ...
                    Picker("Final Report grade", selection: $grade) {
                        ForEach(CourseGrade.allCases) { Text(LocalizedStringKey($0.rawValue)).tag($0) }
                    }
                    .accessibilityIdentifier("courseGradePicker")
                    Text("Leave this as NG until the final report is available. Current and projected grades come from your course work.")
                        .font(.footnote)
