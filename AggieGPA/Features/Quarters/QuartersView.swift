...
struct QuartersView: View {
    ...
    @State private var hasOpenedScreenshotCourse = false
    ...
    var body: some View {
        NavigationStack(path: $navigationPath) {
            ...
            .navigationDestination(for: CourseRecord.self) { course in
    ...
    private func openScreenshotCourseIfNeeded() {
        guard !hasOpenedScreenshotCourse else { return }
        let arguments = ProcessInfo.processInfo.arguments
        guard arguments.contains("--screenshot-course-detail")
                || arguments.contains("--screenshot-grade-breakdown"),
              let course = courses.first(where: { $0.courseCode == "CHE 002A" && !$0.isDeleted }),
              let term = course.term else { return }
        hasOpenedScreenshotCourse = true
    ...
    var body: some View {
        HStack(spacing: DesignSystem.Spacing.medium) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(verbatim: AppCopy.termName(term, locale: locale)).font(.headline)
