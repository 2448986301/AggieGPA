...
struct GradingPolicyEditorView: View {
    ...
    init(course: CourseRecord, policy: CourseGradingPolicy?, scale: GradeScale?) {
        ...
        _scaleConfirmed = State(initialValue: scale.map { !$0.requiresManualReview } ?? false)
    }
        ...
        NavigationStack {
            Form {
                ...
                Section("Letter Prediction") {
                    ...
                    if useCommonScale {
                        Toggle("I confirm this scale matches the course", isOn: $scaleConfirmed)
                    }
    ...
    private func save() {
        ...
        if [.draft, .needsReview, .failed].contains(savedPolicy.importStatus) {
            savedPolicy.importStatus = .confirmed
        }
        ...
        if useCommonScale {
            ...
            savedScale.requiresManualReview = !scaleConfirmed
