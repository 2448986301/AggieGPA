...
struct SemesterMapView: View {
    ...
    var body: some View {
        ...
        .navigationTitle(Text(verbatim: timelineNavigationTitle))
    ...
    private var overview: some View {
        HStack(alignment: .top, spacing: DesignSystem.Spacing.medium) {
            VStack(alignment: .leading, spacing: 4) {
                if let selectedTerm {
                    Text(verbatim: AppCopy.termName(selectedTerm, locale: locale))
                        .font(.title2.bold())
                } else {
                    Text("Semester")
                        .font(.title2.bold())
    ...
    @ViewBuilder private var filterControls: some View {
        Menu {
            ...
            filterLabel(selectedTerm.map { AppCopy.termName($0, locale: locale) } ?? "Semester", systemImage: "calendar")
        }
    ...
    private func itemNode(_ item: GradeItem) -> some View {
        let highImpact = isHighImpact(item)
        return NavigationLink {
            ...
            HStack(alignment: .top, spacing: DesignSystem.Spacing.small) {
                Image(systemName: statusSymbol(item))
                    .foregroundStyle(highImpact ? DesignSystem.ColorToken.gold : statusColor(item))
                    .frame(width: 22)
                VStack(alignment: .leading, spacing: 3) {
                    HStack {
                        Text(verbatim: item.title)
                            .font(.subheadline.weight(highImpact ? .bold : .semibold))
    ...
    private var timelineNavigationTitle: String {
        let key = ProcessInfo.processInfo.arguments.contains("--screenshot-academic-timeline")
            ? "Academic Timeline"
            : "Semester Map"
        return AppLocalization.string(key, locale: locale)
    }

    private func isHighImpact(_ item: GradeItem) -> Bool {
        guard let category = item.category, category.weight > 0, item.possiblePoints > 0 else { return false }
        ...
        let impact: Decimal
        if category.calculationMode == .equalItems, !categoryItems.isEmpty {
            impact = category.weight / Decimal(categoryItems.count)
        } else {
            let possiblePoints = categoryItems.reduce(Decimal.zero) { $0 + $1.possiblePoints }
            guard possiblePoints > 0 else { return false }
            impact = category.weight * item.possiblePoints / possiblePoints
        }
        return impact >= 10
    }

    @ViewBuilder
    private func timelineActions(for item: GradeItem) -> some View {
        Menu {
            if !hasRecordedScore(item) && item.status != .submitted {
                Button("Mark Complete", systemImage: "checkmark.circle") {
                    markComplete(item)
                }
                .accessibilityIdentifier("timelineMarkComplete-\(item.id.uuidString)")
            }
            ...
            .accessibilityIdentifier("timelineRecordScore-\(item.id.uuidString)")
            Button("Edit Assignment Details", systemImage: "pencil") {
                editingItem = item
            }
            .accessibilityIdentifier("timelineEditItem-\(item.id.uuidString)")
            ...
            .accessibilityIdentifier("timelineReminder-\(item.id.uuidString)")
        } label: {
        ...
        .accessibilityIdentifier("timelineActions-\(item.id.uuidString)")
    }
    ...
    private var progressDescription: String {
        ...
        return AppCopy.termName(selectedTerm, locale: locale)
    }
    ...
    private func statusSymbol(_ item: GradeItem) -> String {
        if item.earnedPoints != nil || item.status == .graded || item.status == .submitted { return "checkmark.circle.fill" }
    ...
    private func statusColor(_ item: GradeItem) -> Color {
        if item.earnedPoints != nil || item.status == .graded || item.status == .submitted { return .green }
    ...
    private func statusLabel(_ item: GradeItem) -> LocalizedStringKey {
        if item.earnedPoints != nil || item.status == .graded || item.status == .submitted { return "Completed" }
