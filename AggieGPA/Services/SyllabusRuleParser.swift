...
nonisolated enum SyllabusRuleParser {
    static func parse(_ text: String, extractionConfidence: Double = 1) -> SyllabusParseResult {
        ...
        for line in lines {
            ...
            if lower.contains("best") && (lower.contains(" of ") || lower.contains("out of")) {
                reasons.append("A best-N-of-M rule is mentioned and requires manual review.")
            }
            ...
            if lower.contains("late") && (lower.contains("homework") || lower.contains("assignment") || lower.contains("work")) {
                reasons.append("A late-work policy is mentioned; confirm the deadline and penalty.")
            }
            if lower.contains("attendance") {
                reasons.append("An attendance policy is mentioned; confirm how it affects the grade.")
            }
            if (lower.contains("minimum") || lower.contains("at least")) && (lower.contains("exam") || lower.contains("final") || lower.contains("midterm")) {
                reasons.append("A minimum-exam requirement is mentioned and requires manual review.")
            }
            if lower.contains("bonus") || lower.contains("extra credit") || lower.contains("extra-credit") {
                reasons.append("A bonus or extra-credit rule is mentioned; confirm its limit and denominator.")
            }
            ...
            if lower.contains("scale") && parseGradeBoundaries([line]).isEmpty {
                reasons.append("A custom grading scale is mentioned but its boundaries were not fully recognized.")
            }
            if (lower.contains("either") || lower.contains("or instead")) && (lower.contains("grade") || lower.contains("exam")) {
                reasons.append("Alternative grading rules conflict and require manual review.")
            }
        }
