...
`Localizable.xcstrings` is now the single source of truth. Xcode extracted 463 active SwiftUI strings from the app, and 276 matching entries from the retired `zh-Hans.lproj/Localizable.strings` were mechanically preserved. The resulting catalog currently contains 366 translated zh-Hans entries and 97 entries that still need product-language review or translation.
...
- Review each of the 97 untranslated catalog entries, prioritizing settings, syllabus import, advanced calculations, errors, and destructive confirmations.
